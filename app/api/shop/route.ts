...
export async function POST(request: NextRequest) {
  try {
    ...
    if (body.updateType === "no-navigation-change" && body.changes) {
      const { changes } = body

      // Handle rename operations
      if (changes.renameOnly) {
        shopConfig.shopTabs = shopConfig.shopTabs.map((tab) => {
          if (changes.renameOnly[tab.name]) {
            return { ...tab, name: changes.renameOnly[tab.name] }
          }
          return tab
        })
      }

      // Handle adding items to existing tabs
      if (changes.addItems) {
        shopConfig.shopTabs = shopConfig.shopTabs.map((tab) => {
          if (changes.addItems[tab.name]) {
            return {
              ...tab,
              items: [...(tab.items || []), ...changes.addItems[tab.name]],
            }
          }
          return tab
        })
      ...
      if (changes.updateSectionContent) {
        shopConfig.shopTabs = shopConfig.shopTabs.map((tab) => {
          if (changes.updateSectionContent[tab.name]) {
            return {
              ...tab,
              content: { ...tab.content, ...changes.updateSectionContent[tab.name] },
            }
          }
          return tab
        })
